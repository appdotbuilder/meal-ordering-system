"""Service layer for meal ordering system."""
...
class DishService:
    ...
    @staticmethod
    def update_stock(dish_id: int, quantity_change: int) -> bool:
        ...
        with get_session() as session:
            ...
            new_quantity = dish.stock_quantity + quantity_change
            if new_quantity < 0:
                return False

            dish.stock_quantity = new_quantity
            dish.updated_at = datetime.utcnow()
            session.commit()
            return True
...
class CartService:
    ...
    @staticmethod
    def update_cart_item(user_id: int, cart_item_id: int, update_data: CartItemUpdate) -> Optional[CartItem]:
        ...
        with get_session() as session:
            ...
            if not dish or update_data.quantity > dish.stock_quantity:
                return None
...
class OrderService:
    ...
    def create_order_from_cart(user_id: int, order_data: OrderCreate) -> Optional[Order]:
        ...
        with get_session() as session:
            ...
            # Create order items and update stock only if order has ID
            if order.id:
...
class ReportService:
    ...
    @staticmethod
    def get_department_order_summary() -> List:
        """Get order summary by department."""
        # Simplified version - returns empty list for now
        return []

    @staticmethod
    def get_dish_order_summary() -> List:
        """Get order summary by dish."""
        # Simplified version - returns empty list for now
        return []
